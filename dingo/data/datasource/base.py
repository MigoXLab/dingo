...
class DataSource:
    """Represents the source of a dataset used in Dingo Tracking, providing
    information such as cloud storage location, delta table name / version,
    etc."""
    ...
    def load(self) -> Iterable:
        """Loads files / objects referred to by the Datasource. For example,
        depending on the type of :py:class:`Datasource
        <dingo.data.datasource.Datasource>`, this may download source CSV files
        from S3 to the local filesystem, load a source Delta Table as a Spark
        DataFrame, etc.
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Obtains a JSON-compatible dictionary representation of the
        Datasource.
    ...
    def to_json(self) -> str:
        """Obtains a JSON string representation of the :py:class:`Datasource
        <dingo.data.datasource.Datasource>`.
