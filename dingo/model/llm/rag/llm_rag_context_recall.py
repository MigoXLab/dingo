...
class LLMRAGContextRecall(BaseOpenAI):
 ...
 输出: """
    ...
    def build_messages(cls, input_data: Data) -> List:
        ...
        """
        # 提取字段
        question = input_data.prompt or getattr(input_data, "question", "")
        # Context Recall 需要 expected_output 而不是实际的 answer
        expected_output = getattr(input_data, "expected_output", "")
        if not expected_output:
            # 如果没有 expected_output，尝试使用 content 或 answer
            expected_output = input_data.content or getattr(input_data, "answer", "")
