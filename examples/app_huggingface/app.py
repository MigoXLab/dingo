...
def dingo_demo(
    dataset_source, input_path, uploaded_file, data_format, column_content, rule_list, prompt_list, model, key, api_url
):
    if not data_format:
...
if __name__ == '__main__':
    ...
    with gr.Blocks() as demo:
            ...
            with gr.Column():
                with gr.Column():
                    dataset_source = gr.Dropdown(
                        choices=['hugging_face', 'local'], value='hugging_face', label='dataset [source]'
                    )
                    ...
                    uploaded_file = gr.File(label='upload file', visible=False)
                    ...
                    rule_list = gr.CheckboxGroup(
                        choices=rule_options, value=['RuleAbnormalChar', 'RuleAbnormalHtml'], label='rule_list'
                    )
                    prompt_list = gr.CheckboxGroup(choices=prompt_options, label='prompt_list')
                    model = gr.Textbox(
                        placeholder='If want to use llm, please input model, such as: deepseek-chat', label='model'
                    )
                    key = gr.Textbox(
        ...
        dataset_source.change(fn=update_input_components, inputs=dataset_source, outputs=[input_path, uploaded_file])
