...
class TestRuleDocFormulaRepeat:
    def test_rule_doc_formula_repeat(self):
        ...
        if isinstance(res.error_type, dict):
            res.error_type = ResTypeInfo(**res.error_type)
        assert res.error_type.label == ["QUALITY_BAD_SIMILARITY.RuleDocFormulaRepeat"]
        assert res.error_type.metric == ["RuleDocFormulaRepeat"]
        assert res.error_type.reason == ["Formula has too many consecutive repeated characters, total repeat length: 130, found 1 repeat patterns"]
    ...
    def test_rule_unsafe_words(self):
        ...
        assert 'av' not in tmp.error_type.reason
        assert 'b' not in tmp.error_type.reason
        assert 'java' in tmp.error_type.reason
